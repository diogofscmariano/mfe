...
export function fakeAPIFetch(options) {
  ...
  } else if (options.url.includes("config")) {
    return handleFakeRequestForConfig(options.url);
  }
  return Promise.resolve({});
}

function handleFakeRequestForConfig(url) {
  return fakeNetwork(wrapWithData(config));
}
